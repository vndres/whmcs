...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-xl font-semibold text-slate-50">Nuevo Addon</h1>
        ...
        <form action="{{ route('admin.addons.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" 
                       ...
                       placeholder="Ej: IP Dedicada" required>
            </div>

            <div>
                <label class="block text-xs font-medium text-slate-300 mb-1">Descripción</label>
                <textarea name="description" rows="2" 
                          class="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" class="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100">
                        <option value="recurring">Recurrente (Se cobra con el ciclo del producto)</option>
            ...
            <div>
                <label class="block text-xs font-medium text-slate-300 mb-1">Costo de Instalación (Opcional)</label>
                <div class="relative">
                    <span class="absolute left-3 top-2 text-slate-500">$</span>
                    <input type="number" step="0.01" name="setup_fee" value="{{ old('setup_fee', '0.00') }}" 
                           class="w-full bg-slate-900/80 border border-slate-700 rounded-xl pl-6 pr-3 py-2 text-sm text-slate-100">
                </div>
            </div>
            ...
            <div class="pt-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" checked class="rounded bg-slate-900 border-slate-700 text-emerald-500">
                    <span class="text-sm text-slate-300">Activo (Disponible para asignar a productos)</span>
                </label>
            </div>
